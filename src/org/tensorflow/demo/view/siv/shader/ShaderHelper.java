package org.tensorflow.demo.view.siv.shader;
...
public abstract class ShaderHelper {
	...
	public abstract void calculate(int bitmapWidth, int bitmapHeight,
	...
	public void init(Context context, AttributeSet attrs, int defStyle) {
		if (attrs != null) {
			...
			borderColor = typedArray.getColor(
					R.styleable.ShaderImageView_siBorderColor, borderColor);
			borderWidth = typedArray.getDimensionPixelSize(
					R.styleable.ShaderImageView_siBorderWidth, borderWidth);
			borderAlpha = typedArray.getFloat(
					R.styleable.ShaderImageView_siBorderAlpha, borderAlpha);
			square = typedArray.getBoolean(
					R.styleable.ShaderImageView_siSquare, square);
			typedArray.recycle();
		}
	...
	public Bitmap calculateDrawableSizes() {
		...
		if (bitmap != null) {
			...
			if (bitmapWidth > 0 && bitmapHeight > 0) {
				...
				calculate(bitmapWidth, bitmapHeight, width, height, scale,
